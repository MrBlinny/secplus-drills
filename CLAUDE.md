...
| `content/cram-decks.json` | 413 free-recall cram cards: 321 acronyms + 92 port cards | Generated from `tools/build-cram.mjs`. Acronyms are parsed from the objectives acronym table, so they cannot drift from it. Ports are IANA facts, written for this repo. **Supplementary** — never a credited answer alone, and excluded from the coverage audit. |
...
- `mode` — `drill · coverage · weak · exam · pairs · blank · pbq · cram`, plus `baseline` and `practice-test` for imported answers, which never count toward the study projection.
- **`cram` rows carry `domain: null` deliberately.** Acronyms and ports span the whole blueprint, so attributing them to a domain would move the weighted projection on evidence that is not about that domain. They count in raw accuracy and in the review scheduler, and nowhere else. Do not give them a domain to make the dashboard look busier.
...
- `ACR-*`, `PORT-P-*` (protocol→port), `PORT-N-*` (port→protocol) — cram deck cards.
...
- **The cram grader tolerates typos but never siblings.** `server/cram.js` accepts an answer one edit away from the target, *except* when the typed string is some other card's answer in the same deck. HTTP is one character from HTTPS, LDAP from LDAPS, IMAP from IMAPS — and that character is the whole point of the question. If you extend the grader, keep that rule.
